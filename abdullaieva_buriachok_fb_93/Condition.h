...
public:
	...
	int get_type() const//сколько параметров с типом NAME
	{
		if (this->left[0] == '`' && this->right[0] == '`')//VALUE ? VALUE
		{
			return 0;//0 параметров с типом NAME
		}

		else if (this->left[0] == '`' || this->right[0] == '`')//NAME ? VALUE
		{
			return 1;//1 параметр с типом NAME
		}

		else//NAME ? NAME
		{
			return 2;//2 параметра с типом NAME
		}
		
		//return this->right[0] == '`' ? (this->left[0] == '`' ? 0 : 1) : 2;
	}
	...
	bool get_value(string left_value, string right_value) const//NAME ? NAME
	{
		...
		{
			return left_value.length() < right_value.length();
		}
