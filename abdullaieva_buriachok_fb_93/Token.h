...
bool lexer_analysis(string code, Token_Type token_types_array[], const int token_types_size, deque<Token>& tokens_list)
{
	...
	while (position < code.length())
	{
		...
		if (check)
		{
			string ex = "error at position " + to_string(position);
...
void parser_analysis(Database& database, const Array<Token>& tokens_array)
{
	...
	if (tokens_array[0].type == Type::CREATE)
	{
		...
		{
			string ex = "unexpected token at position " + to_string(tokens_array_size - 1);
		...
		for (int i = 4; i < tokens_array_size - 1; i++)
		{
			...
			{
				string ex = "unexpected token at position " + to_string(i);
	...
	if (tokens_array[0].type == Type::SELECT)
	{
		...
		{
			string ex = "unexpected token at position " + to_string(position_from + 1);
		...
		if (position_from + 2 < tokens_array_size && tokens_array[position_from + 2].type == Type::JOIN)
		{
			...
			{
				string ex = "unexpected token at position " + to_string(position_from + 3);
			...
			if (position_join + 2 < tokens_array_size && tokens_array[position_join + 2].type == Type::ON)
			{
				...
				{
					string ex = "unexpected token at position " + to_string(position_join + 3);
				...
				{
					string ex = "unexpected token at position " + to_string(position_join + 4);
				...
				{
					string ex = "unexpected token at position " + to_string(position_join + 5);
		...
		if (position_where < tokens_array_size)
		{
			...
			{
				string ex = "unexpected token at position " + to_string(position_where);
			...
			{
				string ex = "unexpected token at position " + to_string(position_where + 1);
			...
			{
				string ex = "unexpected token at position " + to_string(position_where + 2);
			...
			{
				string ex = "unexpected token at position " + to_string(position_where + 3);
