...
bool Table::insert(const Array<string>& values)
{
	...
	{
		cerr << "wrong number of parameters" << endl;	throw;
	}
	...
	for (int i = 0; i < indexes_size; i++)
	{
		...
		{
			cerr << column_name << " already has value " << value << endl;	throw;
		}
	}
...
void Table::erase(const int row_index)
{
	...
	{
		cerr << "unknown row index " << row_index << endl;	throw;
	}
...
int Table::erase(const Condition& condition)
{
	...
	if (condition_type == 1)//NAME ? VALUE
	{
		...
		{
			cerr << "unknown column " << column_name << endl;	throw;
		}
		...
		for (auto node = this->indexes_array[index_index->second].begin(); node != this->indexes_array[index_index->second].end(); ++node)
		{
			if (condition_operation[0] == '<' && !is_equal && node->first < condition_value)
			{
	...
	if (condition_type == 2)//NAME ? NAME
	{
		...
		{
			cerr << "unknown column " << column_name1 << endl;	throw;
		}

		if (column_index2 == this->columns_tree.end())
		{
			cerr << "unknown column " << column_name2 << endl;	throw;
		}
...
void Database::create(string table_name, const Array<string>& columns, const Array<string>& indexes)
{
	...
	{
		cerr << "table " << table_name << " already exists" << endl;	throw;
	}
	...
	for (int i = 0; i < columns_size; i++)
	{
		for (int j = 0; j < columns_size; j++)
		{
			...
			{
				cerr << "few columns have the same name" << endl;	throw;
			}
		}
	}
...
Table& Database::operator[](string table_name)
{
	...
	{
		cerr << "unknown table " << table_name << endl;	throw;
	}
